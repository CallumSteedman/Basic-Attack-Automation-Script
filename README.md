...
The script will input and subsequently read a file containing IP addresses each seperated by a line, for each IP address found in the list the script will scan the chosen ports on that host. The script will then attempt to bruteforce a certain subset of selected services if they are running on the host.
